...
- v2.3.0：**上线问候附今日天气**——配置 `weatherLat`/`weatherLon`（+`weatherName`）后，上线播报自动附带 Open-Meteo 天气（状况/当前温度与体感/今日高低温/湿度/风力，6s 超时降级无天气）；`deploy.ps1` 新增**公网 IP 自动定位**（ip-api.com 免 key，zh-CN）自动填天气字段，可用 `-WeatherLat/-WeatherLon/-WeatherCity` 手动钉住或 `-NoWeather` 关闭，已有配置保留；修复 deploy 配置写入带 BOM 导致 daemon 启动即崩的隐患（改为 UTF-8 无 BOM）
